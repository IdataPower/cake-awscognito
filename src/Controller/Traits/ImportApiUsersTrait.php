...
trait ImportApiUsersTrait
{
    ...
    protected function _import($headers = [])
    {
        ...
        if(empty($csv_data)){
            $this->Flash->warning(__d('PowerSystem/CognitoSDK', 'Nothing to import.'));
            return;
        }
        ...
        if($data_rows > $max_rows){
            $this->Flash->error(__d('PowerSystem/CognitoSDK',
        ...
        if($result){
            $this->Flash->success(__d('PowerSystem/CognitoSDK', '{0} API Users imported successfully', count($result)));
        }else{
            $this->Flash->error(__d('PowerSystem/CognitoSDK', 'An error occurred while trying to import the API Users. Please try again.'));
        }
