<?php
namespace PowerSystem\CognitoSDK\Controller\Traits;
...
trait AwsCognitoTrait
{
    ...
    public function resetPassword($id = null)
    {
        ...
        if(!$api_user->aws_cognito_synced){
            $this->Flash->error(__d('PowerSystem/CognitoSDK', 'The API User is not synced with AWS Cognito'));
        }elseif(!$api_user->aws_cognito_attributes['email_verified']){
            $this->Flash->error(__d('PowerSystem/CognitoSDK', 'The user email must be verified before resetting the password'));
        }elseif ($this->ApiUsers->resetCognitoPassword($api_user)) {
            $this->Flash->success(__d('PowerSystem/CognitoSDK', 'The password has been reset'));
        } else {
            $this->Flash->error(__d('PowerSystem/CognitoSDK', 'The password could not be reset. If the user has not yet changed their temporary password, they must do so before the password can be reset'));
        }
    ...
    public function resendInvitationEmail($id = null)
    {
        ...
        if ($this->request->is(['patch', 'post', 'put'])) {
            ...
            if ($this->ApiUsers->resendInvitationEmail($api_user, $email)) {
                $this->Flash->success(__d('PowerSystem/CognitoSDK', 'The Api User has been saved'));
                return $this->redirect(['action' => 'index']);
            }
            $this->Flash->error(__d('PowerSystem/CognitoSDK', 'The Api User could not be saved'));
        }
