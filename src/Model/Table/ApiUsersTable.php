<?php
namespace PowerSystem\CognitoSDK\Model\Table;
...
use Exception;
use PowerSystem\CognitoSDK\Model\Traits\AwsCognitoSaveTrait;
...
class ApiUsersTable extends Table
{
	...
	public function initialize(array $config)
    {
        ...
        $this->addBehavior('PowerSystem/CognitoSDK.CognitoSDK', Configure::read('CognitoSDK'));
        $this->addBehavior('PowerSystem/CognitoSDK.ImportApiUsers');
        ...
        $this->addBehavior('PowerSystem/S3UploadSDK.S3UploadSDK', [
            'avatar_file_name' => [
                'fields' => [
                    'dir'          => 'avatar_file_dir',
    ...
    public function validationDefault(Validator $validator)
    {
        ...
        $validator
            ...
            ->inList('role', array_keys($this->getRoles()),
                sprintf(
                    __d('PowerSystem/CognitoSDK', 'Must be one of the following values: %s'),
                    implode(
    ...
    public function getRoles()
    {
        ...
        if(!Configure::check('ApiUsers.roles')){
            throw new Exception(__d('PowerSystem/CognitoSDK', 'ApiUsers.roles setting is invalid'));
        }

        $roles = Configure::read('ApiUsers.roles');

        foreach ($roles as $role => $name) {
            if(is_numeric($role)) throw new Exception(__d('PowerSystem/CognitoSDK', 'The ApiUsers.roles array should be entirely associative'));
        }
