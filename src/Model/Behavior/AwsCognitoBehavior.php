<?php
namespace PowerSystem\CognitoSDK\Model\Behavior;
...
class AwsCognitoBehavior extends Behavior
{
        ...
        $validator
            ->add('email', [
                'email' => [
                    'rule' => 'email',
                    'last' => true,
                    'message' => __d('PowerSystem/CognitoSDK', 'This field must be a valid email address')
                ]
            ])
    ...
    public function buildRules(Event $event, RulesChecker $rules)
    {
        $rules->add($rules->isUnique(['aws_cognito_username']), '_isUnique', [
            ...
            'message' => __d('PowerSystem/CognitoSDK', 'Username already exists')
        ]);

        $rules->add($rules->isUnique(['email']), '_isUnique', [
            'errorField' => 'email',
            'message' => __d('PowerSystem/CognitoSDK', 'Email already exists')
        ]);
        ...
        $rules->add(function($entity, $options){
            ...
            'errorField' => 'email',
            'message' => __d('PowerSystem/CognitoSDK', 'The email cannot be directly modified')
        ]);
        ...
        $rules->add(function($entity, $options){
            ...
            'errorField' => 'aws_cognito_username',
            'message' => __d('PowerSystem/CognitoSDK', 'The cognito username cannot be edited')
        ]);
        ...
        $rules->add(function($entity, $options){
            ...
            'message' => __d('PowerSystem/CognitoSDK', 'The cognito id cannot be edited')
        ]);
    ...
    {
        if($entity->isNew()){
            throw new Exception(__d('PowerSystem/CognitoSDK', 'Cannot edit email of an nonexistent user.'));
        }

        if(empty($entity->aws_cognito_username)){
            throw new Exception(__d('PowerSystem/CognitoSDK', 'The user does not have a Cognito Username.'));
        }
    ...
    public function resendInvitationEmail(EntityInterface $entity, $new_email)
    {
        ...
        if($entity->isNew()){
            throw new Exception(__d('PowerSystem/CognitoSDK', 'You must create the entity before trying to resend the invitation email'));
        }
    ...
    protected function createCognitoClient()
    {
        if(!$this->getConfig('UserPool.id')){
            throw new Exception(__d('PowerSystem/CognitoSDK', 'the AWS User Pool ID has not been set.'));
        }

        if(!$this->getConfig('AccessKeys.id', false)
        || !$this->getConfig('AccessKeys.secret', false)){
            throw new Exception(__d('PowerSystem/CognitoSDK', 'the AWS credentials have not been set.'));
        }
    ...
    protected function titleForUserStatus($status)
    {
        switch ($status) {
            case 'UNCONFIRMED':
                return __d('PowerSystem/CognitoSDK', 'Unconfirmed');
            case 'CONFIRMED':
                return __d('PowerSystem/CognitoSDK', 'Confirmed');
            case 'ARCHIVED':
                return __d('PowerSystem/CognitoSDK', 'Archived');
            case 'COMPROMISED':
                return __d('PowerSystem/CognitoSDK', 'Compromised');
            case 'RESET_REQUIRED':
                return __d('PowerSystem/CognitoSDK', 'Reset Required');
            case 'FORCE_CHANGE_PASSWORD':
                return __d('PowerSystem/CognitoSDK', 'Force Change Password');
        }
        return __d('PowerSystem/CognitoSDK', 'Unknown');
    }
    ...
    protected function descriptionForUserStatus($status)
    {
        switch ($status) {
            case 'UNCONFIRMED':
                return __d('PowerSystem/CognitoSDK', 'The user cannot sign in until the user account is confirmed.');
            case 'CONFIRMED':
                return __d('PowerSystem/CognitoSDK', 'The user account is confirmed and the user can sign in.');
            case 'ARCHIVED':
                return __d('PowerSystem/CognitoSDK', 'User is no longer active.');
            case 'COMPROMISED':
                return __d('PowerSystem/CognitoSDK', 'User is disabled due to a potential security threat.');
            case 'RESET_REQUIRED':
                return __d('PowerSystem/CognitoSDK', 'The user account is confirmed, but the user must request a code and reset their password before they can sign in.');
            case 'FORCE_CHANGE_PASSWORD':
                return __d('PowerSystem/CognitoSDK', 'The user account is confirmed and the user can sign in using a temporary password, but on first sign-in, the user must change their password to a new value before doing anything else.');
        }
        return null;
    }
    ...
    protected function awsExceptionToEntityErrors(AwsException $exception, EntityInterface $entity)
    {
        if($exception->getAwsErrorCode() === 'AliasExistsException'){
            $entity->setError('email', [
                'isUniqueCognito' => __d('PowerSystem/CognitoSDK', 'The email is not unique in AWS Cognito')
            ]);
            return $entity;
        }
        ...
        if($exception->getAwsErrorCode() === 'UsernameExistsException'){
            $entity->setError('aws_cognito_username', [
                'isUniqueCognito' => __d('PowerSystem/CognitoSDK', 'The username is not unique in AWS Cognito')
            ]);
            return $entity;
        }
